...
public class CameraPathFrame extends CgApplication {
	...
	private void createScene() {
		String[] filenames = { "meshes/bunny.obj", "meshes/cow.obj",
				"meshes/teddy.obj", "meshes/big_porsche.ply",
		...
		for (int x = 0; x < RESOLUTION; x++) {
			...
			for (int z = 0; z < RESOLUTION; z++) {
				...
				// Select random node
				ITriangleMesh mesh = new TriangleMesh(meshes.get((int) (Math
						.random() * meshes.size())));
				...
				CgNode transformationNode = new CgNode(transformation,
						"transformation");
				transformation.addTranslation(VectorMatrixFactory.newIVector3(
						xPos, 0, zPos));
	...
	public void saveImageAsJpeg(BufferedImage image, String filename) {
		try {
			...
			imageWriter.write(imageMetaData, new IIOImage(image, null, null),
					null);
			ios.close();
			imageWriter.dispose();
		} catch (IOException e) {
			Logger.getInstance()
					.error("Failed to write JPEG image " + filename);
		}
	}
	...
	public static void main(String[] args) {
		...
		AppLauncher.getInstance().create(new CameraPathFrame());
