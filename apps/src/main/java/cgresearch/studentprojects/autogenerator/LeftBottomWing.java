...
public class LeftBottomWing {
  ...
  public LeftBottomWing(Vector center, double hoehe, double laenge,
  ...
  public void setLeft(double bwlrh, double bwlrv, double bwtph, double bwtpv,
      double bwlr2h, double bwlr2v) {
    Curve curve = new Curve(new BasisFunctionBezier(), 3);

    curve.setControlPoint(0,
        VectorFactory.createVector3(A.get(0) - bwtph, A.get(1) + bwtpv, 0));

    curve.setControlPoint(1, VectorFactory.createVector3(A.get(0) - bwlr2h,
        A.get(1) + offset + bwlr2v, 0));

    curve.setControlPoint(2, VectorFactory.createVector3(A.get(0) - bwlrh,
        A.get(1) + hoehe / 2 + bwlrv, 0));
  ...
  public void setBottom(double wingMitteBreite, double bwbh, double bwbv,
    ...
    curve.setControlPoint(0,
        VectorFactory.createVector3(C.get(0), C.get(1) - breite, 0));

    curve.setControlPoint(1,
        VectorFactory.createVector3(D.get(0) - bwbh, D.get(1) + bwbv, 0));

    curve.setControlPoint(2, VectorFactory
        .createVector3(A.get(0) + offset - bwb2h, A.get(1) + bwb2v, 0));
