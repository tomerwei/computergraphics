...
public class RightBottomWing {
	...
	public void setRight(double bwlrh, double bwlrv, double bwtph, double bwtpv, double bwlr2h, double bwlr2v) {
		...
		curve.setControlPoint(3, VectorFactory.createVector3(D.get(0) + bwtph, D.get(1) + bwtpv, 0));

		curve.setControlPoint(2, VectorFactory.createVector3(D.get(0) + bwlr2h, D.get(1) + offset + bwlr2v, 0));

		curve.setControlPoint(1, VectorFactory.createVector3(D.get(0) + bwlrh, D.get(1) + hoehe / 2 + bwlrv, 0));

		curve.setControlPoint(0, VectorFactory.createVector3(C.get(0), C.get(1), 0));
	...
	public void setBottom(double wingMitteBreite, double bwbh, double bwbv, double bwtph, double bwtpv, double bwb2h,
		...
		curve.setControlPoint(3, VectorFactory.createVector3(B.get(0), B.get(1) - breite, 0));

		curve.setControlPoint(2, VectorFactory.createVector3(A.get(0) + bwbh, A.get(1) + bwbv, 0));

		curve.setControlPoint(1, VectorFactory.createVector3(D.get(0) + offset + bwb2h, D.get(1) + bwb2v, 0));

		curve.setControlPoint(0, VectorFactory.createVector3(D.get(0) + bwtph, D.get(1) + bwtpv, 0));
