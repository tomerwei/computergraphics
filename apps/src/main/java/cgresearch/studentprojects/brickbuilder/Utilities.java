...
public class Utilities {
	...
	/**
	 * Creates a texture from the color array and adds it with a unique id to the resource manager.
	 * @param colors
	...
	public static String createTextureFromColors(List<IColorRGB> colors) {
		...
		for (int x = 0; x < colors.size(); x++)
			drawQuad(tex, (x % MAXCOLORS) * CELLSIZE, (x / MAXCOLORS) * CELLSIZE, CELLSIZE,
					colors.get(x).getAsInteger());	
	 ...
	 * @param cellSize	size of the box in the texture
	 */
	public static void addBox(ITriangleMesh mesh, IVector3 dim, IVector3 pos,
			int colorX, int colorY, int sizeX, int sizeY, int cellSize) {		
		...
		for (int i = 0; i < cube.getNumberOfTriangles(); i++) {
			Triangle t = cube.getTriangle(i);			
			Triangle u = new Triangle(vertex2vertex.get(t.getA()), 
					vertex2vertex.get(t.getB()), 
					vertex2vertex.get(t.getC()),
					texCoord2texCoord.get(t.getTextureCoordinate(0)), 
					texCoord2texCoord.get(t.getTextureCoordinate(1)), 
    ...
    public static ITriangleMesh createCube(int colorX, int colorY, int sizeX, int sizeY, int cellSize) {
        ...
        int c00 = mesh.addTextureCoordinate(VectorMatrixFactory.newIVector3(
        		colorX*cellSize*1.0 / sizeX,
        		colorY*cellSize*1.0 / sizeY, 0)); // up left
        int c01 = mesh.addTextureCoordinate(VectorMatrixFactory.newIVector3(
        		colorX*cellSize*1.0 / sizeX,
        		(colorY+1)*cellSize*1.0 / sizeY, 0)); // down left
        int c10 = mesh.addTextureCoordinate(VectorMatrixFactory.newIVector3(
        		(colorX+1)*cellSize*1.0 / sizeX,
        		colorY*cellSize*1.0 / sizeY, 0)); // up right
        int c11 = mesh.addTextureCoordinate(VectorMatrixFactory.newIVector3(
        		(colorX+1)*cellSize*1.0 / sizeX,
