 ...
 * by Stefanie Mueller, Tobias Mohr, Kerstin Guenther, Johannes Frohnhofen, Patrick Baudisch
 * 
		...
		for (int z = 0; z < cloud.getResolutions().getZ(); z++) {
			for (int y = 0; y < cloud.getResolutions().getY(); y++) {
				for (int x = 0; x < cloud.getResolutions().getX(); x++) {
					...
					IVector3 ll = location.add(VectorMatrixFactory.newIVector3(
							xRes * x, yRes * y, zRes * z));
					IVector3 ur = location.add(VectorMatrixFactory.newIVector3(
							xRes * (x+1), yRes * (y+1), zRes * (z+1)));					
					if (vertexInBox(mesh, ll, ur)) cloud.setVoxelAt(p, VoxelType.INTERIOR);		
				}
			}
		}
