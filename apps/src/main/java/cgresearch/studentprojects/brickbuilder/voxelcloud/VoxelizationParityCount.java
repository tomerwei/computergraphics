...
public class VoxelizationParityCount implements IVoxelizationAlgorithm {
	...
	public IVoxelCloud transformMesh2Cloud(final ITriangleMesh mesh, int resolutionAxisX, IVector3 voxelScale) {
		...
		if (mesh.getMaterial().hasTexture()) {
			...
			if (cgTex.getTextureImage() != null) {
				...
				String absoluteTextureFilename = ResourcesLocator.getInstance()
	                .getPathToResource(cgTex.getTextureFilename());
		...
		// the algorithm needs a NxNxN space, so we look for the max and set it for all axis
			...
			for (int y = 0; y < resolutionAxisAll; y++) {
				for (int x = 0; x < resolutionAxisAll; x++) {
					...
					if (count[index] > 0) {
						...
						IVector3 pos = locationCenter.add(VectorMatrixFactory.newIVector3(
						...
						if (tex != null) {
							...
							if (axiz > -1) {
								cloud.setVoxelColor(vec, getColorFromUV(mesh, tex, colorPoints[axiz][index],
										colorTriangles[axiz][index]));
							}
						}
	 ...
	 * @param t		triangle
	 * @param ds	bounding box data to store at
	 */
	private void calcBoundingBox(ITriangleMesh mesh, Triangle t, double[][] ds) {
		for (int i = 0; i < 3; i++) {
			IVector3 v = mesh.getVertex(t.get(i)).getPosition();
			if (v.get(0) < ds[0][0]) ds[0][0] = v.get(0);
			if (v.get(1) < ds[0][1]) ds[0][1] = v.get(1);
			if (v.get(2) < ds[0][2]) ds[0][2] = v.get(2);
			if (v.get(0) > ds[1][0]) ds[1][0] = v.get(0);
			if (v.get(1) > ds[1][1]) ds[1][1] = v.get(1);
			if (v.get(2) > ds[1][2]) ds[1][2] = v.get(2);
		}
	}
	 ...
	 * @param mesh		mesh
	 * @param rayLoc	ray location
	 * @param rayDir 	ray direction
	 * @param bbs		triangle bounding boxes
	 * @return
	 */
	private Map<IVector3, Integer> findIntersects(ITriangleMesh mesh, IVector3 rayLoc, IVector3 rayDir, double[][][] bbs) {
		Map<IVector3, Integer> intersects = new HashMap<IVector3, Integer>();		
		for (int i = 0; i < mesh.getNumberOfTriangles(); i++) {
			// check triangle bounding box
			if (!checkBoundingBox(rayLoc, rayDir, bbs[i])) continue;
			// get ray triangle intersect point
			IVector3 r = rayTriangleIntersect(rayLoc, rayDir, mesh, mesh.getTriangle(i));
			if (r != null) intersects.put(r, i);
		}		
		return intersects;
	}
	...
	private boolean checkBoundingBox(IVector3 rayLoc, IVector3 rayDir, double[][] bb) {
		...
		if (rayLoc.get(a) < bb[0][a] || rayLoc.get(b) < bb[0][b]) return false;
		if (rayLoc.get(a) > bb[1][a] || rayLoc.get(b) > bb[1][b]) return false;
		return true;
	}
	 ...
	 * @param res				resolution for all axiz
	 * @param step				voxel length for each axiz
	 * @param locCenter			voxel 0,0 center
	 * @param count				count array
	 * @param bbs				bounding boxes of the triangles
	 * @param colorPoints		triangle point to calc the color of
	 * @param colorTriangles	triangle to calc the color of
	 */
	private void castRay(ITriangleMesh mesh, final int axiz, int res, double[] step, IVector3 locCenter, int[] count, 
		...
		for (pos[a] = 0; pos[a] < res; pos[a]++) {
			for (pos[b] = 0; pos[b] < res; pos[b]++) {
				...
				IVector3 loc = locCenter.add(VectorMatrixFactory.newIVector3(
						(axiz == 0 ? -step[0] : pos[0] * step[0]), 
						(axiz == 1 ? -step[1] : pos[1] * step[1]),
						(axiz == 2 ? -step[2] : pos[2] * step[2])));				
				Map<IVector3, Integer> intersectsMap = findIntersects(mesh, loc, dir, bbs);
				// broken ray? no vote
				if (intersectsMap.size() % 2 != 0) continue;
				...
				for (pos[axiz] = 0; pos[axiz] < res; pos[axiz]++) {
					...
					// set point and triangle of the nearest intersect to calc the color from it
	...
	/**
	 * Returns the point where the ray intersects the triangle or null if they dont.
	 * Source: http://www.lighthouse3d.com/tutorials/maths/ray-triangle-intersection/
	 * 
	 * @param rayP	ray destination
	 * @param rayD	ray direction
	 * @param mesh	triangle mesh
	 * @param t		triangle
	 * @return		intersect point or null
	 */
	private IVector3 rayTriangleIntersect(IVector3 rayP, IVector3 rayD, ITriangleMesh mesh, Triangle t) {
		// e1 = v1 - v0
		...
		//
		if (w > 0.00001) return rayP.add(rayD.multiply(w));
	...
	private static IColorRGB getColorFromUV(ITriangleMesh mesh, BufferedImage tex, IVector3 point, int index) {
		// source: http://answers.unity3d.com/questions/383804/calculate-uv-coordinates-of-3d-point-on-plane-of-m.html
		Triangle triangle = mesh.getTriangle(index);
		IVector3[] vertices = new IVector3[] {
				mesh.getVertex(triangle.getA()).getPosition(), 
				mesh.getVertex(triangle.getB()).getPosition(),
				mesh.getVertex(triangle.getC()).getPosition()
		};
		IVector3[] texCoords = new IVector3[] {
				mesh.getTextureCoordinate(triangle.getTextureCoordinate(0)), 
		...
		int color = tex.getRGB((int) ((tex.getWidth() - 1) * u),
				tex.getHeight() - 1 - (int) ((tex.getHeight() - 1) * v));
