...
public class ScannerApplication extends CgApplication
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_INIT)) {
      scanner =
          Scanner.getInstance(scannerGui.getPort(),
              scannerGui.getPointsPerLine(),
              scannerGui.getAngle());
      scanner.initStartPosition();
    } else if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_TEST)) {
      scanner.fireTest();
    } else if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_LASER_ON)) {
      scanner.laserOn();
    } else if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_LASER_OFF)) {
      scanner.laserOff();
    } else if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_SAVE_FILE)) {
      scanner.saveLastPointCloud(scannerGui
          .getSaveFilename());
    } else if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_LOAD_FILE)) {
      scanner.loadLastPointCloud(getCgRootNode(),
          scannerGui.getLoadFilename());
    } else if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_DEBUG_ANGLE)) {
      scanner.rotateAngle(scannerGui.getDebugAngle());
    } else if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_DEBUG_HEIGHT)) {
      scanner.moveHeight(scannerGui.getDebugHeight());
    } else if (e.getActionCommand().equals(
        ScannerGui.ACTION_COMMAND_SCAN)) {
      new Thread(new Runnable() {
        ...
        public void run() {
          ...
          getCgRootNode().addChild(
              new CgNode(pointCloud, "Scan"));
          scanner.startScan(pointCloud,
              scannerGui.getPointsPerLine(),
              scannerGui.getAngle());
        }
      }).start();
  ...
  public static void main(String[] args) {
    ...
    JoglSwingUserInterface ui =
        new JoglSwingUserInterface(app, joglFrame);
    ui.registerApplicationGUI(scannerGui);
    ScannerInformationGui tinkerforgeGui =
        new ScannerInformationGui();
    ui.registerApplicationGUI(tinkerforgeGui);
    CalibrationGui calibrationGui = new CalibrationGui();
    ui.registerApplicationGUI(calibrationGui);
