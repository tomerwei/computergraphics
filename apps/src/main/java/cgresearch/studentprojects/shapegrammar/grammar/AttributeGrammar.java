...
public class AttributeGrammar {
   ...
   */
  public void setAttributes(VirtualFormTree formTree) {
  ...
  private void reposition(Node<IVirtualForm> node) {
    ...
    if (!(shape instanceof VirtualCuboid)) {
      ...
      VirtualPoint position =
          new VirtualPoint(oldPosition.getX() - maxNegativX,
              oldPosition.getY(), oldPosition.getZ() - maxNegativZ);
   ...
   */
  private void setAttributes(Node<IVirtualForm> node) {
    GrammarRule nodeGrammar =
        ruleTree.getNodeByName(node.getData().getName()).getData();
   ...
   *          the new position
   */
  private void setPosition(Node<IVirtualForm> node) {
    IVirtualForm shape = node.getData();
    if (!(shape instanceof VirtualCuboid)) {
      for (Node<IVirtualForm> childNode : node.getChildren()) {
        ...
        switch (shape.getPosition().getDirection()) {
          case Top:
            newChildPoint =
                new VirtualPoint(point.getX() + childPoint.getX(),
                    point.getY(), point.getZ() - childPoint.getY(),
                    point.getDirection());
            break;
          case Left:
            newChildPoint =
                new VirtualPoint(point.getX(),
                    point.getY() + childPoint.getY(), point.getZ()
                        + childPoint.getX(), point.getDirection());
            break;
          case Right:
            newChildPoint =
                new VirtualPoint(point.getX(),
          ...
          case Front:
            newChildPoint =
                new VirtualPoint(point.getX() + childPoint.getX(), point.getY()
                    + childPoint.getY(), point.getZ(), point.getDirection());
            break;
          case Back:
            newChildPoint =
                new VirtualPoint(point.getX() - childPoint.getX(), point.getY()
      ...
      if (position.getDirection() != null) {
        ...
        BuildingSettings buildSettings =
        ...
        switch (position.getDirection()) {
          case Top:
            ...
            buildSettings.setHeight(buildSettings.getHeight() + height);
            break;
          case Left:
            newCubePosition =
                new VirtualPoint(position.getX() - cube.getLendth(),
                    position.getY(), position.getZ(), position.getDirection());
            width = cube.getLendth();
            height = cube.getHeight();
            lendth = cube.getWidth();
            buildSettings.setWidth(buildSettings.getWidth() + width);
            break;
          case Right:
            newCubePosition = position;
            width = cube.getLendth();
          ...
          case Front:
            newCubePosition =
                new VirtualPoint(position.getX(), position.getY(),
                    position.getZ() + cube.getLendth(), position.getDirection());
            width = cube.getWidth();
            height = cube.getHeight();
            lendth = cube.getLendth();
            buildSettings.setLength(buildSettings.getLength() + lendth);
            break;
          case Back:
            newCubePosition =
                new VirtualPoint(position.getX() - cube.getLendth(),
        ...
        node.setData(splitgrammar.split(cube, new VirtualFormTree())
            .getRootNode().getData());
        for (Node<IVirtualForm> childNode : node.getChildren()) {
          ...
          VirtualShape newChildShape =
              new VirtualShape(childShape.getName(), side.getPosition(),
