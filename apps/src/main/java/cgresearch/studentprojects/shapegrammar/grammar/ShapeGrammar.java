...
/**
 * The Class ShapeGrammar is the main Grammar instance and use the splitgrammar and attributegrammar.
...
public class ShapeGrammar {
	 ...
	 */
	public VirtualFormTree performGrammar(){
		...
		VirtualCuboid building = new VirtualCuboid(ruleTree.getRootNode().getData().getName(), new VirtualPoint(buildingSettings.getX(), 0, buildingSettings.getZ()), buildingSettings.getWidth(), buildingSettings.getHeight(), buildingSettings.getLength());
		SplitGrammar splitGrammar = new SplitGrammar();
		AttributeGrammar attributeGrammar = new AttributeGrammar();
		VirtualFormTree formTree = splitGrammar.split(building, BuildingSettingManager.getInstance().getActualSettings().getFormTree());
		attributeGrammar.setAttributes(formTree);
		return formTree;
	}
}
