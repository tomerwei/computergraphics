...
public class SplitGrammar {
	 ...
	 */
	public VirtualFormTree split(IVirtualForm building, BuildingSettings settings) {
		settings.getFormTree().addRootNode(building);
		split(settings.getFormTree().getRootNode(), settings);
		return settings.getFormTree();
	}
	 ...
	 *            the node
	 */
	private void split(Node<IVirtualForm> node, BuildingSettings settings) {
		IVirtualForm form = node.getData();
		VirtualMethod splitMethod = settings.getRuleTree().getNodeByName(form.getName()).getData().getMethod();
		if (splitMethod != null) {
			if (splitMethod.getName().equals("extrude")) {
				form = new VirtualCuboid(form.getName(), form.getPosition(), form.getWidth(), form.getHeight(),
						Double.parseDouble(splitMethod.getParameters()));
				node.setData(form);
	...
	/**
	 * Shape perform in given direction Split shapes in the given parts
	 * 
	 * @param shape
	 *            the shape
	 * @param node
	 ...
	 */
	private void shapePerform(VirtualShape shape, Node<IVirtualForm> node, VirtualMethod splitMethod,
			String direction) {
		if (splitMethod.getName().equals("split")) {
			...
			while (tempRest > 0 && iterator <= ruleValuePairs.size() - 1) {
				...
				if (actualPair.getValues() != null) {
					...
					VirtualPoint virtualPos = (direction.equals("x") ? new VirtualPoint(position, 0, 0)
							: new VirtualPoint(0, position, 0));
					if (value.equals("~")) {
						...
						if (tempRest >= 0) {
							...
							IVirtualForm childShape = new VirtualShape(actualPair.getRulename(), virtualPos, width,
									height);
			...
			for (Node<IVirtualForm> childNode : node.getChildren()) {
				...
				childShape.setPosition(pos);
				if (direction.equals("x")) {
					pos = new VirtualPoint(pos.getX() + childShape.getWidth(), pos.getY(), pos.getZ(),
							pos.getDirection());
				} else {
					pos = new VirtualPoint(pos.getX(), pos.getY() + childShape.getHeight(), pos.getZ(),
							pos.getDirection());
				}
			}
		}
	...
	/**
	 * Cuboid perform. Split Cuboid in 5 Parts
	 *
	 * @param cuboid
	 *            the Cuboid
	 * @param node
	 ...
	 *            the split method
	 */
	private void cuboidPerform(VirtualCuboid cuboid, Node<IVirtualForm> node, VirtualMethod splitMethod) {
		List<IRulenameValuePair> ruleValuePairs = splitMethod.getDelegation();
		for (IRulenameValuePair ruleValuePair : ruleValuePairs) {
			...
			for (String side : cubeSides) {
				...
				VirtualShape shape = new VirtualShape(rule, cubeSideShape.getPosition(), cubeSideShape.getWidth(),
						cubeSideShape.getHeight());
				Node<IVirtualForm> childNode = new Node<IVirtualForm>();
