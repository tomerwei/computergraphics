...
public class FrustumTestFrame extends CgApplication {
  ...
  public FrustumTestFrame(CgNode root, double nearDistance, double farDistance) {
    ...
    if (objects.size() > 0) {
      ...
      for (int i = 0; i < objects.size(); i++) {
        objects.get(i).setVisible(false); // TODO hier muss eigentlich false
                                          // stehen, true nur fuer Debugging
      }

      // getCgRootNode().addChild(new CgNode(octreeScene, "oScene"));

      // for(int i = 0; i < visibleNodes.size(); i++){
      // getCgRootNode().addChild(new CgNode(visibleNodes.get(i),
      // "octreeScene"));
      // }

      // ITriangleMesh frustum_parallel =
      // test.getFrustumMesh(test.get_corners());
  ...
  public void addVisibleElementsToScene(ViewFrustumCulling vfc, OctreeNode<Integer> tree, CgNode node,
