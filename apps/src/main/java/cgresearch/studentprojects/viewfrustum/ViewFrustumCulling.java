...
//        calc_planes_of_frustum(1, 3, 0.1);  // is not in view frustum
    ...
    public ViewFrustumCulling getTest(){
        ...
        planes[0]=vfc.calcPlane(VectorMatrixFactory.newIVector3(-1.0, 1.0, 1.0),VectorMatrixFactory.newIVector3(-1.0, -1.0, 1.0),(VectorMatrixFactory.newIVector3(1.0, -1.0, 1.0))); // near
        planes[1]=vfc.calcPlane(VectorMatrixFactory.newIVector3(-1.0, 1.0, -1.0),VectorMatrixFactory.newIVector3(-1.0, -1.0, -1.0),(VectorMatrixFactory.newIVector3(1.0, -1.0, -1.0))); // far
        planes[1].setNormal(planes[1].getNormal().multiply(-1.0));
        planes[2]=vfc.calcPlane(VectorMatrixFactory.newIVector3(-1.0, 1.0, 1.0),VectorMatrixFactory.newIVector3(-1.0, -1.0, 1.0),(VectorMatrixFactory.newIVector3(-1.0, -1.0, -1.0))); // left
        planes[2].setNormal(planes[2].getNormal().multiply(-1.0));
        planes[3]=vfc.calcPlane(VectorMatrixFactory.newIVector3(1.0, 1.0, 1.0),VectorMatrixFactory.newIVector3(1.0, -1.0, 1.0),(VectorMatrixFactory.newIVector3(1.0, -1.0, -1.0))); // right
        planes[4]=vfc.calcPlane(VectorMatrixFactory.newIVector3(-1.0, 1.0, 1.0),VectorMatrixFactory.newIVector3(1.0,1.0, 1.0),(VectorMatrixFactory.newIVector3(1.0, 1.0, -1.0))); // top
        planes[5]=vfc.calcPlane(VectorMatrixFactory.newIVector3(-1.0, -1.0, 1.0),VectorMatrixFactory.newIVector3(1.0, -1.0, 1.0),(VectorMatrixFactory.newIVector3(1.0, -1.0, -1.0))); // bottom
     ...
     * @param nearDistance Distanz zur nahen Ebene 
     * @param farDistance Distanz zur fernen Ebene 
     * @param viewRatio Breite des Frustums
     */
    public void calcPlanesOfFrustum(double nearDistance, double farDistance, double viewRatio){
        ...
        farBottomRight = farCenter.add(this.up.multiply(farHeight* 0.5).subtract(this.cameraRight.multiply(farWidth * 0.5)));
...
//        System.out.println("farTopRight = " + farTopRight);
        farTopLeft = farCenter.subtract(this.up.multiply(farHeight* 0.5).subtract(this.cameraRight.multiply(farWidth * 0.5)));
        cornerPoints[ftl] = farTopLeft;
//        System.out.println("farTopLeft = " + farTopLeft);

        nearBottomRight = nearCenter.add(this.up.multiply(nearHeight* 0.5).subtract(this.cameraRight.multiply(nearWidth * 0.5)));
...
//        System.out.println("nearBottomLeft = " + nearBottomLeft);
        nearTopRight = nearCenter.subtract(this.up.multiply(nearHeight* 0.5).add(this.cameraRight.multiply(nearWidth* 0.5)));
        cornerPoints[ntr] = nearTopRight;
//        System.out.println("nearTopRight = " + nearTopRight);
        nearTopleft = nearCenter.subtract(this.up.multiply(nearHeight* 0.5).subtract(this.cameraRight.multiply(nearWidth* 0.5)));
...
//        System.out.println("bottom_plane normal = "+ bottom_plane.getNormal() );
     ...
     * @param point_one erster Eckpunkt
     * @param point_two zweiter Eckpunkt
     * @param point_three dritter Eckpunkt
     ...
     * @param point zu berechnender Punkt
     ...
     * @param obj Objekt, das geprueft werden soll
    ...
    public int isObjectInFrustum(ICgNodeContent obj){
        ...
        if(obj.getClass() == OctreeNode.class){
            IVector3 ur = VectorMatrixFactory.newIVector3(((OctreeNode)obj).getLowerLeft().get(0)+ ((OctreeNode)obj).getLength(), ((OctreeNode)obj).getLowerLeft().get(1)+ ((OctreeNode)obj).getLength(), ((OctreeNode)obj).getLowerLeft().get(2)+ ((OctreeNode)obj).getLength());
        ...
        for(int i=0; i < frustum.length; i++){
            ...
            for(int j=0; j < corner_points.length && (in == 0 || out == 0); j++){
                ...
                // pruefe, ob sie innerhalb oder außerhalb liegt
     ...
     * @param corner_points Eckpunkte des View Frustums
    ...
    private IVector3[] calcCornerPointsOfBoundingBox(BoundingBox b_box){
        // nach http://www.lighthouse3d.com/tutorials/view-frustum-culling/geometric-approach-testing-boxes/
        ...
        IVector3 lnl, lfl, lfr, lnr, ufr, unr, unl, ufl; // corner points of bounding box
     ...
     * @param octreeNode der zu pruefende Octree
     * @param toDraw Liste, der die im Frustum liegenden OctreeNodes hinzugefuegt werden
     * @return Liste mit den im Frustum liegenden OctreeNodes
     */
    public ArrayList<OctreeNode<Integer>> checkOctree(OctreeNode<Integer> octreeNode, ArrayList<OctreeNode<Integer>> toDraw){
    ...
    /**
     * wird aufgerufen, wenn ein OctreeNode innerhalb des Frustums liegt, um an seine Leaf Nodes zu kommen
     * @param node der OctreeNode, der komplett im Frustum liegt
     * @param toDraw ArrayList, der die leaf nodes hinzugefuegt werden
     * @return
     */
    public ArrayList<OctreeNode<Integer>> traversalOctreeNode(OctreeNode<Integer> node, ArrayList<OctreeNode<Integer>> toDraw){
    ...
    /**
     * fuegt zu einem TriangleMesh alle Triangles hinzu, die der uebergebene Octree indexiert
     * @param nodesInFrustum OctreeNodes im View Frustum
     * @param content der Content, von dem extrahiert werden soll
