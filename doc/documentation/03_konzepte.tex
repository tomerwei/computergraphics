...
In jeder Szene kann Alpha-Blending (Transparenz) verwendet werden. Zunächst muss man dazu in dem Szenen-Wurzelknoten \verb+CgRootNode+ Blending aktivieren: 

\begin{verbatim}
getCgRootNode().setUseBlending(true);
\end{verbatim}

Für jedes Objekt der Szene, das Alpha-Blending verwendet soll, setzt man die Transparenz individuell in dessen Material:

\begin{verbatim}
<Objekt>.getMaterial().setTransparency(0.5);
\end{verbatim}

Gültig sind alle Werte zwischen 0 (vollständig transparent) und 1 (vollständig opaque).

Um Transparenzen verwenden zu können, muss das Blending vom verwendeten Shader unterstützt werden. Dies ist aktuell für die Shader \emph{Phong Shading} und \emph{Textur} der Fall.