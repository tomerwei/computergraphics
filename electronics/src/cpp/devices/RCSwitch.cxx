 ...
 * @return char[13]
 */
char* RCSwitch::getCodeWordB(int nAddressCode, int nChannelCode,
		boolean bStatus) {
	int nReturnPos = 0;
 ...
 */
char* RCSwitch::getCodeWordC(char sFamily, int nGroup, int nDevice,
		boolean bStatus) {
	static char sReturn[13];
	int nReturnPos = 0;

	if ((byte) sFamily < 97 || (byte) sFamily > 112 || nGroup < 1 || nGroup > 4
			|| nDevice < 1 || nDevice > 4) {
		return '\0';
	}

	char* sDeviceGroupCode = dec2binWzerofill((nDevice - 1) + (nGroup - 1) * 4,
			4);
	char familycode[16][5] = { "0000", "F000", "0F00", "FF00", "00F0", "F0F0",
			"0FF0", "FFF0", "000F", "F00F", "0F0F", "FF0F", "00FF", "F0FF",
			"0FFF", "FFFF" };
	for (int i = 0; i < 4; i++) {
...
//	std::cout << "RCSwitch::sendTriState" << std::endl;
...
void RCSwitch::transmit(int nHighPulses, int nLowPulses) {
	...
	//std::cout << "RCSwitch::transmit" << std::endl;
	...
	if (this->nTransmitterPin != -1) {
		...
		std::cout << "RCSwitch::transmit(): Invalid nTransmitterPin."
				<< std::endl;
	}
}
/**
...
void RCSwitch::sendT0() {
	...
	//std::cout << "RCSwitch::sendT0" << std::endl;
...
void RCSwitch::sendT1() {
	...
	//std::cout << "RCSwitch::sendT1" << std::endl;
