...
public class BoundingBox {
	...
	public void add(IVector3 p) {
		...
		if (!isInitialized) {
			...
			ll = VectorMatrixFactory.newIVector3(
					Math.min(ll.get(MathHelpers.INDEX_0),
							p.get(MathHelpers.INDEX_0)),
					Math.min(ll.get(MathHelpers.INDEX_1),
							p.get(MathHelpers.INDEX_1)),
					Math.min(ll.get(MathHelpers.INDEX_2),
							p.get(MathHelpers.INDEX_2)));

			ur = VectorMatrixFactory.newIVector3(
					Math.max(ur.get(MathHelpers.INDEX_0),
							p.get(MathHelpers.INDEX_0)),
					Math.max(ur.get(MathHelpers.INDEX_1),
							p.get(MathHelpers.INDEX_1)),
					Math.max(ur.get(MathHelpers.INDEX_2),
							p.get(MathHelpers.INDEX_2)));
		}
	}
	...
	public double getMaxExtend() {
		...
		return Math.max(diagonal.get(0),
				Math.max(diagonal.get(1), diagonal.get(2)));
	}
	...
	public void transform(IMatrix3 rotation, IVector3 translation) {
		...
		for (int x = 0; x < 2; x++) {
			for (int y = 0; y < 2; y++) {
				for (int z = 0; z < 2; z++) {
					points.add(ll.add(VectorMatrixFactory.newIVector3(x
							* extend.get(0), y * extend.get(1),
							z * extend.get(2))));
				}
			}
		}
	...
	public void transform(Transformation transformation) {
		...
		for (int i = 0; i < points.size(); i++) {
			...
			points.get(i).copy(
					transformation.getTransformation().multiply(p).toVector3());
		}
		isInitialized = false;
		for (IVector3 p : points) {
			add(p);
