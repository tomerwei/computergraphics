...
public class Matrix3 implements IMatrix3 {
   ...
   */
  private final IVector3[] rows =
      { VectorMatrixFactory.newIVector3(), VectorMatrixFactory.newIVector3(), VectorMatrixFactory.newIVector3() };
  ...
  public IMatrix4 makeHomogenious() {
    ...
    row2.set(3, 0);
    return VectorMatrixFactory.newIMatrix4(row0, row1, row2, VectorMatrixFactory.newIVector4(0, 0, 0, 1));
  }
  ...
  public IVector3 getColumn(int columnIndex) {
    return VectorMatrixFactory.newIVector3(get(0, columnIndex), get(1, columnIndex), get(2, columnIndex));
  }
  ...
  public IMatrix3 getInverse() {
    ...
    double h = get(2, 1);
    double i = get(2, 2);
    IMatrix3 inverse = VectorMatrixFactory.newIMatrix3(e * i - f * h, c * h - b * i, b * f - c * e, f * g - d * i,
  ...
  @Override
  public IMatrix3 multiply(double factor) {
    IMatrix3 result = new Matrix3();
    for (int row = 0; row < 3; row++) {
      for (int column = 0; column < 3; column++) {
