...
public class Curve extends ICgNodeContent {
	...
	public Vector eval(double t) {
		...
		for (int i = 0; i <= getDegree(); i++) {
			p = p.add(controlPoints[i].multiply(basisFunctions.eval(i, t, getDegree())));
		}
		return p;
	}
	...
	public Vector derivative(double t) {
		return eval(t + FINITE_DIFFERENCES_STEP_WIDTH / 2.0).subtract(eval(t - FINITE_DIFFERENCES_STEP_WIDTH / 2.0))
