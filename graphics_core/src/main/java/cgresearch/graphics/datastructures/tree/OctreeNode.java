	...
	public void subdivide() {
		if (children != null) {
			Logger.getInstance().error(
					"Octree:subdivide(): children array must be null.");
			return;
		}
		...
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				for (int k = 0; k < 2; k++) {
					IVector3 childLowerLeftCorner = lowerLeftCorner
							.add(VectorMatrixFactory.newIVector3(i
									* childLength, j * childLength, k
									* childLength));
					OctreeNode<T> childNode = new OctreeNode<T>(
							childLowerLeftCorner, childLength);
