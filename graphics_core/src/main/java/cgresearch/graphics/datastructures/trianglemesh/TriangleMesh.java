...
public class TriangleMesh extends ITriangleMesh {
	...
	public TriangleMesh(ITriangleMesh other) {
		if (other instanceof TriangleMesh) {
			...
			Logger.getInstance()
					.error("Cannot use triangle mesh copy constructor with ITriangleMesh instances.");
		}
	}
	 ...
	 * (nicht-Javadoc)
	 * 
	 * @see edu.cg1.exercises.mesh.ITriangleMesh#addVertex(double, double,
	 * double)
	 */
	@Override
	...
	public void computeTriangleNormals() {
		...
		for (int i = 0; i < getNumberOfTriangles(); i++) {
			...
			Vertex pA = getVertex(t.getA());
			Vertex pB = getVertex(t.getB());
			Vertex pC = getVertex(t.getC());
	 ...
	 * 
	 * @see
	 * edu.cg1.exercises.shader.IAdvancedTriangleMesh#getNumberOfTextureCoordinates
	 * ()
	 */
	@Override
	...
	public void computeVertexNormals() {
		for (int vertexIndex = 0; vertexIndex < getNumberOfVertices(); vertexIndex++) {
			getVertex(vertexIndex).getNormal().copy(
	...
	public void fitToUnitBox() {
		...
		double scale = Math.max(
				Math.max(diagonal.get(MathHelpers.INDEX_0),
						diagonal.get(MathHelpers.INDEX_1)),
				diagonal.get(MathHelpers.INDEX_2));
		for (int i = 0; i < vertices.size(); i++) {
			vertices.get(i)
					.getPosition()
					.copy(vertices.get(i).getPosition().subtract(center)
							.multiply(1.0 / scale));
		}
	}
	...
	public void invertFaceNormals() {
		for (int vertexIndex = 0; vertexIndex < getNumberOfVertices(); vertexIndex++) {
			getVertex(vertexIndex).setNormal(
					getVertex(vertexIndex).getNormal().multiply(-1));
		}
		for (int triangleIndex = 0; triangleIndex < getNumberOfTriangles(); triangleIndex++) {
			getTriangle(triangleIndex).setNormal(
					getTriangle(triangleIndex).getNormal().multiply(-1));
		}
	}
	 ...
	 * 
	 * @see
	 * edu.haw.cg.datastructures.trianglemesh.ITriangleMesh#unite(edu.haw.cg
	...
	public void unite(ITriangleMesh otherMesh) {
		...
		// Copy triangles
		for (int triangleIndex = 0; triangleIndex < otherMesh
				.getNumberOfTriangles(); triangleIndex++) {
			Triangle copyTriangle = new Triangle(
					otherMesh.getTriangle(triangleIndex));
