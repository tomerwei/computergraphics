...
public class ObjFileReader {
	...
	/**
	 * Mapping between the index in the OBJ file and the index in the vertex
	 * list in the current triangle mesh. Newly created for each mesh in the OBJ
	 * file.
	 */
	 ...
	 * coordinates list in the current triangle mesh. Newly created for each
	 * mesh in the OBJ file.
	 */
	...
	/**
	 * Remember the directory of the input filename in order to read the
	 * material library.
	 */
		...
		try {
			...
			if (is == null) {
				Logger.getInstance().error(
						"Failed to create input stream for file " + filename);
				return meshes;
			}
			...
			Logger.getInstance().exception("Failed to read mesh from OBJ file",
					e);
		}

		for (int i = 0; i < meshes.size(); i++) {
			Logger.getInstance().debug(
					"Read " + meshes.get(i).getNumberOfVertices()
							+ " vertices, "
							+ meshes.get(i).getNumberOfTriangles()
							+ " triangles and "
							+ meshes.get(i).getNumberOfTextureCoordinates()
							+ " texture coordinates.");
		...
		if (components.length == 2) {
			...
			if (mat != null) {
				...
				if (textureFilename.length() > 0) {
					CgTexture texture = new CgTexture(directory
							+ textureFilename);
					String id = ResourceManager.getTextureManagerInstance()
							.generateId();
					ResourceManager.getTextureManagerInstance().addResource(id,
							texture);
				...
				if (diffuseColor != null) {
					currentMesh.getMaterial()
							.setReflectionDiffuse(diffuseColor);
				}
			}
		}
	}
	...
	private void parseMaterialFile(String line) {
		...
		String[] components = splitEmptySpace(line);
		if (components.length == 2) {
			materials = materialIO.readMaterialFile(directory + "/"
					+ components[1]);
		}
	}
		...
		if (allCoords.length == 4) {
			...
			for (int i = 0; i < 3; i++) {
				...
				if (vertexIndexMapping.get(vertexIndices[i]) == null) {
					vertexIndexMapping.put(vertexIndices[i], currentMesh
							.addVertex(vertices.get(vertexIndices[i])));
				}
				...
				if (textureCoordIndexMapping.get(texCoordIndices[i]) == null) {
					if (textureCoordinates.size() > 0
							&& texCoordIndices[i] >= 0) {
						textureCoordIndexMapping
								.put(texCoordIndices[i],
										currentMesh
												.addTextureCoordinate(textureCoordinates
														.get(texCoordIndices[i])));
					} else {
						textureCoordIndexMapping.put(texCoordIndices[i], -1);
					}
				}
			}
			currentMesh.addTriangle(new Triangle(vertexIndexMapping
					.get(vertexIndices[0]), vertexIndexMapping
					.get(vertexIndices[1]), vertexIndexMapping
					.get(vertexIndices[2]), textureCoordIndexMapping
					.get(texCoordIndices[0]), textureCoordIndexMapping
					.get(texCoordIndices[1]), textureCoordIndexMapping
					.get(texCoordIndices[2])));
			...
			currentMesh.addTriangle(new Triangle(vertexIndexMapping
					.get(vertexIndices[0]), vertexIndexMapping
					.get(vertexIndices[2]), vertexIndexMapping
					.get(vertexIndices[3]), textureCoordIndexMapping
					.get(texCoordIndices[0]), textureCoordIndexMapping
					.get(texCoordIndices[2]), textureCoordIndexMapping
					.get(texCoordIndices[3])));
		}
	}
	...
	private void parseVertex(String strLine) {
		...
		if (components.length == 4) {
			...
			vertices.add(new Vertex(VectorMatrixFactory.newIVector3(x, y, z),
