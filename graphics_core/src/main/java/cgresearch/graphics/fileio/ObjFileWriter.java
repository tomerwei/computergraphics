...
public class ObjFileWriter {
	...
	public void writeToFile(String filename, ITriangleMesh mesh) {
		...
		Logger.getInstance().message(
				"Successfully wrote OBJ file " + filename
						+ " with material lib file " + materialLibFilename
						+ ".");
	...
	private String createMaterialLibFilename(String filename, ITriangleMesh mesh) {
		...
		String filenameWithoutExtension = fileNameWithoutPath.substring(0,
		...
		try {
			...
			if (mesh.getMaterial().getTextureId() != null) {
				CgTexture texture = ResourceManager.getTextureManagerInstance()
						.getResource(mesh.getMaterial().getTextureId());
			...
			Logger.getInstance().exception(
					"Failed to write OBJ material library file", e);
		}
	 ...
	 */
	private void writeMaterialFile(BufferedWriter writer,
			String materialLibFilename) {
		try {
	 ...
	 */
	private void writeTextureCoordinates(BufferedWriter writer,
			ITriangleMesh mesh) {
		for (int texCoordIndex = 0; texCoordIndex < mesh
				.getNumberOfTextureCoordinates(); texCoordIndex++) {
	...
	private void writeFacets(BufferedWriter writer, ITriangleMesh mesh) {
		for (int facetIndex = 0; facetIndex < mesh.getNumberOfTriangles(); facetIndex++) {
			Triangle triangle = mesh.getTriangle(facetIndex);
			try {
				String line = "f ";
				for (int i = 0; i < 3; i++) {
					line += (triangle.get(i) + 1);
					line += (triangle.getTextureCoordinate(i) >= 0) ? "/"
							+ (triangle.getTextureCoordinate(i) + 1) : "";
					line += " ";
				}
				writer.write(line + "\n");
			} catch (IOException e) {
	...
	private void writeVertices(BufferedWriter writer, ITriangleMesh mesh) {
		...
		for (int vertexIndex = 0; vertexIndex < mesh.getNumberOfVertices(); vertexIndex++) {
			...
			try {
				writer.write("v " + vertex.getPosition().get(0) + " "
						+ vertex.getPosition().get(1) + " "
	...
	private void writeHeader(BufferedWriter writer, ITriangleMesh mesh) {
		try {
			...
			writer.write("# " + calendar.get(Calendar.DAY_OF_MONTH) + "."
					+ calendar.get(Calendar.MONTH) + "."
					+ calendar.get(Calendar.YEAR) + ", "
					+ calendar.get(Calendar.HOUR_OF_DAY) + ":"
					+ calendar.get(Calendar.MINUTE) + "\n");
			writer.write("# #vertices:" + mesh.getNumberOfVertices()
					+ ", #texture coordinates: "
					+ mesh.getNumberOfTextureCoordinates() + ", #triangles: "
