...
public class CgGlslShader {
	...
	public static String readShaderSource(String shaderFilename) {
		...
		String absoluteShaderFilename = ResourcesLocator.getInstance()
				.getPathToResource(shaderFilename);
	...
	public static void saveShader(String source, String absoluteFilename) {
		...
		try {
			...
			Logger.getInstance().exception(
					"Failed to write shader source " + absoluteFilename, e);
		} catch (IOException e) {
			Logger.getInstance().exception(
					"Failed to read shader source " + absoluteFilename, e);
		}
	}
}
