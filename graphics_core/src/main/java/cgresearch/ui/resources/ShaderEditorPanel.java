...
public class ShaderEditorPanel extends JPanel implements ActionListener {
	 ...
	 * Constructor
	 */
	public ShaderEditorPanel(CgGlslShader shader, CgGlslShader.ShaderType type,
			IGlslShaderCompiler compiler) {
		this.type = type;
		this.shader = shader;
		...
		// Shader type caption
		String caption = (type == CgGlslShader.ShaderType.VERTEX) ? "Vertex shader"
				: "Fragment shader";
		...
		buttonFilename = new JButton(
				getShortFilename(shader.getShaderSourceFilename(type)));
	...
	private String getShortFilename(String shaderSourceFilename) {
		...
		if (shaderSourceFilename.length() > MAX_LENGTH) {
			return shaderSourceFilename.substring(shaderSourceFilename.length()
					- MAX_LENGTH);
		}
		return null;
	}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand().equals(ACTION_COMMAND_COMPILE)) {
			if (compiler == null) {
				...
				//compiler.compile(JoglShader.getGlShaderType(type),
						//textAreaXSource.getText());
			}
	...
	private void selectShaderFilename() {
		...
		if (fileChooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
			String absoluteFilename = fileChooser.getSelectedFile()
					.getAbsolutePath();
			...
			buttonFilename.setText(getShortFilename(shader
					.getShaderSourceFilename(type)));
		}
	}
	...
	private void saveSourceToFile() {
		...
		String absoluteFilename = ResourcesLocator.getInstance()
				.getPathToResource(shader.getShaderSourceFilename(type));
		if (absoluteFilename == null) {
			// Let user choose file
			JFileChooser fileChooser = new JFileChooser();
			if (fileChooser.showSaveDialog(null) == JFileChooser.APPROVE_OPTION) {
				absoluteFilename = fileChooser.getSelectedFile()
						.getAbsolutePath();
			}
		}
		if (absoluteFilename != null) {
			CgGlslShader
					.saveShader(textAreaXSource.getText(), absoluteFilename);
