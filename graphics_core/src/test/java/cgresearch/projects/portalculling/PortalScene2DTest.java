...
public class PortalScene2DTest {
	...
	private PortalScene2D createDummyScene() {
		...
		scene.clear();
		scene.addNode(VectorFactory.createVector3(-0.5, 0, -0.5));
		scene.addNode(VectorFactory.createVector3(0.5, 0, -0.5));
		scene.addNode(VectorFactory.createVector3(0.5, 0, 0.5));
		scene.addNode(VectorFactory.createVector3(-0.5, 0, 0.5));
	...
	public void testComputeStartCell() {
		PortalScene2D scene = createDummyScene();
		assertEquals(0, scene.getStartCellIndex(VectorFactory
				.createVector3(0, 0, -0.25)));
		assertEquals(1, scene.getStartCellIndex(VectorFactory
				.createVector3(0, 0, 0.25)));
	}
	...
	public void testCellContainsPoint() {
		...
		assertEquals(
				...
				scene.cellContainsPoint(scene.getCell(0),
						VectorFactory.createVector3(0, 0, -0.25)));
		assertEquals(
				false,
	...
	public void testIntersectRays() {
		Ray2D ray1 = new Ray2D(VectorFactory.createVector3(1, 0, 1),
				VectorFactory.createVector3(4, 0, 2));
		Ray2D ray2 = new Ray2D(VectorFactory.createVector3(2, 0, 3),
				VectorFactory.createVector3(1, 0, -1));
		assertEquals(0.5, ray1.intersect(ray2).lambda1, 1e-5);
		assertEquals(1, ray1.intersect(ray2).lambda2, 1e-5);

		Ray2D ray3 = new Ray2D(VectorFactory.createVector3(1, 0, 1),
				VectorFactory.createVector3(1, 0, 1));
		Ray2D ray4 = new Ray2D(VectorFactory.createVector3(2, 0, 3),
				VectorFactory.createVector3(1, 0, 1));
