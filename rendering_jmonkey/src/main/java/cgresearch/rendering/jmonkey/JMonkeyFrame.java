...
public class JMonkeyFrame implements IRenderFrame<Node>, Observer {
	...
	public JMonkeyFrame(CgApplication application) {
		...
		app = new SimpleApplication() {
			...
			public void simpleInitApp() {
				registerRenderObjectsFactory(new JMonkeyRenderObjectFactoryPointCloud(
						assetManager));
				registerRenderObjectsFactory(new JMonkeyRenderObjectFactoryTriangleMesh(
						assetManager));
				...
				renderObjectManager.update(
						cgApplication.getCgRootNode(),
						CgNodeStateChange.makeAddChild(null,
								cgApplication.getCgRootNode()));
			}

		};

		renderObjectManager = new JMonkeyRenderObjectManager(
				application.getCgRootNode(), app.getRootNode());
	...
	private void updateLights() {
		...
		for (int i = 0; i < cgRootNode.getNumberOfLights(); i++) {
			...
			case DIRECTIONAL:
				...
				directionalLight.setDirection(new Vector3f((float) lightSource
						.getDirection().get(0), (float) lightSource
						.getDirection().get(1), (float) lightSource
						.getDirection().get(2)));
				directionalLight.setColor(new ColorRGBA((float) lightSource
						.getDiffuseColor().get(0), (float) lightSource
						.getDiffuseColor().get(1), (float) lightSource
						.getDiffuseColor().get(2), 1));
				app.getRootNode().addLight(directionalLight);
				break;
			case POINT:
				PointLight pointLight = new PointLight();
				pointLight.setPosition(new Vector3f((float) lightSource
						.getPosition().get(0), (float) lightSource
						.getPosition().get(1), (float) lightSource
						.getPosition().get(2)));
				pointLight.setColor(new ColorRGBA((float) lightSource
						.getDiffuseColor().get(0), (float) lightSource
						.getDiffuseColor().get(1), (float) lightSource
						.getDiffuseColor().get(2), 1));
			...
			default:
				Logger.getInstance().error(
						"Unsupported light type: " + lightSource.getType());
				break;
			}
		}
	 ...
	 * 
	 * @see
	 * edu.haw.cg.rendering.IRenderFrame#registerRenderObjectsFactory(edu.haw
	...
	public IGlslShaderCompiler getShaderCompiler() {
		Logger.getInstance().message(
				"The jMonkey frame does not provide a shader compiler.");
		return null;
	}
}
