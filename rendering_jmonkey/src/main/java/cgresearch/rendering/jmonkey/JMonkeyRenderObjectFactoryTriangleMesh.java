 ...
 * 
 */
public class JMonkeyRenderObjectFactoryTriangleMesh implements
	...
	public Node createRenderObject(Node parentNode, CgNode cgNode) {
		...
		if (triangleMesh.getMaterial().getTextureId() != null) {
			...
			CgTexture cgTexture = cgresearch.graphics.material.ResourceManager
					.getTextureManagerInstance().getResource(
							triangleMesh.getMaterial().getTextureId());
			...
			mat.setColor("Diffuse", new ColorRGBA((float) color.get(0),
					(float) color.get(1), (float) color.get(2), 1));
			// mat.set
			geometry.setMaterial(mat);
		}
	 ...
	 */
	private Geometry createTriangleMeshMesh(AssetManager assetManager,
		...
		for (int triangleIndex = 0; triangleIndex < triangleMesh
			...
			for (int vertexInTriangleIndex = 0; vertexInTriangleIndex < 3; vertexInTriangleIndex++) {
				...
				int texCoordIndex = t
						.getTextureCoordinate(vertexInTriangleIndex);
				Vertex vertex = triangleMesh.getVertex(vertexIndex);
				IVector3 pos = vertex.getPosition();
				IVector3 normal = vertex.getNormal();
				IVector3 color = triangleMesh.getMaterial()
						.getReflectionDiffuse();
				IVector3 texCoord = triangleMesh
						.getTextureCoordinate(texCoordIndex);

				// Position
				vertexBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3] = (float) pos
						.get(0);
				vertexBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3 + 1] = (float) pos
						.get(1);
				vertexBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3 + 2] = (float) pos
						.get(2);

				// Normal
				normalBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3] = (float) normal
						.get(0);
				normalBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3 + 1] = (float) normal
						.get(1);
				normalBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3 + 2] = (float) normal
						.get(2);

				// Color
				colorBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3] = (float) color
						.get(0);
				colorBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3 + 1] = (float) color
						.get(1);
				colorBuffer[triangleIndex * 9 + vertexInTriangleIndex * 3 + 2] = (float) color
						.get(2);

				// Texture coords
				texCoordsBuffer[triangleIndex * 6 + vertexInTriangleIndex * 2] = (float) texCoord
						.get(0);
				texCoordsBuffer[triangleIndex * 6 + vertexInTriangleIndex * 2
						+ 1] = (float) texCoord.get(1);
			}
		}
