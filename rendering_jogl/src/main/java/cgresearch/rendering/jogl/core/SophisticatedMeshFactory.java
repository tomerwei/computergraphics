...
public class SophisticatedMeshFactory {
   ...
   */
  private static ITriangleMesh createSophisticatedMeshMesh(ITriangleMesh triangleMesh) {
    ...
    for (int i = 0; i < triangleMesh.getNumberOfVertices(); i++) {
      sophisticatedMesh
          .unite(TriangleMeshFactory.createSphere(triangleMesh.getVertex(i).getPosition(), 0.05f * scale, 10));
    }
    // Edges
    for (int i = 0; i < triangleMesh.getNumberOfTriangles(); i++) {
      ITriangle triangle = triangleMesh.getTriangle(i);
      for (int j = 0; j < 3; j++) {
        ...
        if (v0Index > v1Index) {
          // Line v0 -> v1
          Line3D line =
              new Line3D(triangleMesh.getVertex(v0Index).getPosition(), triangleMesh.getVertex(v1Index).getPosition());
          sophisticatedMesh.unite(TriangleMeshFactory.createLine3D(line, 10, 0.015));
        }
      }
    }
  ...
  public static JoglRenderNode createSophisticatedMeshNode(ITriangleMesh mesh) {
    ...
    sophisticatedMesh.getMaterial().setReflectionDiffuse(VectorFactory.createVector3(0.6, 0.6, 0.9));
