...
public class JoglTexture {
	...
	public static void use(CgTexture texture, GL2 gl) {
		...
		if (!texture.isLoaded()) {
			...
			if (joglTexture != null) {
				...
				Logger.getInstance()
						.message("Successfully destroided texture.");
			}

			if (texture.getTextureImage() != null) {
				joglTextures.put(
						texture,
						createTexture(gl.getGLProfile(),
								texture.getTextureImage()));
			} else if (texture.getTextureFilename() != null) {
				joglTextures.put(texture,
						createTexture(texture.getTextureFilename()));
			}
		...
		if (joglTextures.get(texture) != null) {
			...
			gl.glBindTexture(GL2.GL_TEXTURE_2D, joglTextures.get(texture)
					.getTextureObject(gl));
	...
	public static Texture createTexture(String textureFilename) {
		...
		String absoluteTextureFilename = ResourcesLocator.getInstance()
				.getPathToResource(textureFilename);
		if (absoluteTextureFilename == null) {
			Logger.getInstance().error(
					"Failed to read texture file " + textureFilename);
			return null;
		}
		try {
			texture = TextureIO.newTexture(new File(absoluteTextureFilename),
					doMipMap);
		} catch (GLException e) {
			Logger.getInstance().exception(
					"Failed to assign texture file " + absoluteTextureFilename,
					e);
		} catch (IOException e) {
			Logger.getInstance().exception(
					"Failed to assign texture file " + absoluteTextureFilename,
		...
		Logger.getInstance().debug(
				"Successfully loaded texture " + absoluteTextureFilename);
		return texture;
	}
