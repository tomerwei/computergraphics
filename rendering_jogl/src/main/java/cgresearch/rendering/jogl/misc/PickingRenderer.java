...
public class PickingRenderer implements Observer {
  ...
  private CgNode createCoordinateSystemNode() {
    ...
    CgNode coordSystemNode = new CgNode(selectedItemTransformation, "picking coordinate system");

    // X
    Arrow arrowX = new Arrow(VectorFactory.createVector3(0, 0, 0), VectorFactory.createVector3(0.3, 0, 0));
    arrowX.getMaterial().setReflectionDiffuse(VectorFactory.createVector3(1, 0, 0));
    arrowX.getMaterial().setShaderId(Material.SHADER_PHONG_SHADING);
    coordSystemNode.addChild(new CgNode(arrowX, "x"));

    // Y
    Arrow arrowY = new Arrow(VectorFactory.createVector3(0, 0, 0), VectorFactory.createVector3(0, 0.3, 0));
    arrowY.getMaterial().setReflectionDiffuse(VectorFactory.createVector3(0, 1, 0));
    arrowY.getMaterial().setShaderId(Material.SHADER_PHONG_SHADING);
    coordSystemNode.addChild(new CgNode(arrowY, "y"));

    // Z
    Arrow arrowZ = new Arrow(VectorFactory.createVector3(0, 0, 0), VectorFactory.createVector3(0, 0, 0.3));
    arrowZ.getMaterial().setReflectionDiffuse(VectorFactory.createVector3(0, 0, 1));
  ...
  public void update(Observable o, Object arg) {
    if (o instanceof Picking) {
      ...
      CgNode currentlySelectedNode = mapPickItemID2CgNode.get(selectedPickingItemId);
      if (currentlySelectedNode != null) {
        currentlySelectedNode.getChildNode(0).getContent().getMaterial().setReflectionDiffuse(COLOR_NOT_SELECTED);
      }

      selectedPickingItemId = Picking.getInstance().getSelectedItemId();
      // Add selection to new node
      CgNode newSelectedNode = mapPickItemID2CgNode.get(Picking.getInstance().getSelectedItemId());
      PickingItem item = Picking.getInstance().getSelectedItem();
      if (newSelectedNode != null) {
        newSelectedNode.getChildNode(0).getContent().getMaterial().setReflectionDiffuse(COLOR_SELECTED);
        Transformation transformation = (Transformation) newSelectedNode.getContent();
