...
/**
 * calculations of the view frustum
 */
...
public class ViewFrustum {
  ...
  // variables for plane calculations
  ...
  public void calculateFrustumPlanes(double nearDistance, double farDistance, double viewRatio) {
    ...
    nearTopleft = (nearCenter.add(this.up.multiply(nearHeight * 0.5))
        .subtract(this.cameraRight.multiply(nearWidth * 0.5)));
  ...
  public TriangleMesh getFrustumMesh(Vector[] cornerPoints) {
    TriangleMesh frustum = new TriangleMesh();
    // near
    frustum.addTriangle(new Triangle(NBR, NBL, NTL));
    frustum.addTriangle(new Triangle(NBR, NTR, NTL));
    // far
    frustum.addTriangle(new Triangle(FBR, FTR, FTL));
    frustum.addTriangle(new Triangle(FBL, FBR, FTL));
    // right
    frustum.addTriangle(new Triangle(FBR, FTR, NTR));
    frustum.addTriangle(new Triangle(FBR, NBR, NTR));
    // left
    frustum.addTriangle(new Triangle(NBL, NTL, FTL));
    frustum.addTriangle(new Triangle(NBL, FBL, FTL));
    // top
    frustum.addTriangle(new Triangle(NBR, NBL, FBL));
    frustum.addTriangle(new Triangle(NBR, FBR, FBL));
    // bottom
    frustum.addTriangle(new Triangle(FTR, NTR, NTL));
    frustum.addTriangle(new Triangle(FTR, FTL, NTL));

    for (int i = 0; i < cornerPoints.length; i++) {
      frustum.addVertex(new Vertex(cornerPoints[i], VectorFactory.createVector3(1, 0, 0)));
    }
    frustum.getMaterial().setTransparency(FRUSTUMTRANSPARENCY);
  ...
  /**
   * checks if a bb is inside or outside the frustum or intersects it(which
   * would be declared as inside)
   * 
   * @param bb
   *          bb whose position should be checked
  ...
  public int isObjectInFrustum(BoundingBox bb) {
    ...
    // because it is not perfomant enough to make meshes partly visible and
    // toggle single triangles,
    ...
    for (int k = 0; k < PLANES; ++k) {
      ...
      for (int j = 0; j < CORNERS; ++j) {
        ...
        // then the mesh must lie outside of the frustum
        if (outside == CORNERS) {
          return OUTSIDE;
        }
      }
  ...
  /**
   * checks if an octreeNode is inside or outside of the frustum or intersects
   * it(which would be declared as inside)
   * 
   * @param oNode
   *          oNode whose position should be checked
   * @return 0 : INSIDE, 1 : OUTSIDE
   */
   ...
   */
  public void addVisibleNode(final OctreeNode<Integer> oNode, final ArrayList<OctreeNode<Integer>> nodesInFrustum) {
  ...
  private Vector[] calcCornerPointsOfBoundingBox(BoundingBox bBox) {
    ...
    Vector lnl, lfl, lfr, lnr, ufr, unr, unl, ufl; // corner points of bounding
                                                   // box
   ...
   * 
   * @param firstIndex
   *          index of the cornerPoints array of the frustum
   * @param secondIndex
   *          index of the cornerPoints array of the frustum
   * @param aCrossB
   *          defines the order of the cross product for the normal calculation
