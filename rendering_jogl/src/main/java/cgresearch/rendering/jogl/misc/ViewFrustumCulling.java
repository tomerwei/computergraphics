...
public class ViewFrustumCulling implements Observer {
    ...
    public ViewFrustumCulling(Camera cam, double nearDistance, double farDistance, CgRootNode rootNode) {
        ...
        cornerPoints = new Vector3[CORNERS];
    ...
    public void calcPlanesOfFrustum(double nearDistance, double farDistance, double viewRatio) {
        ...
        farTopLeft = (farCenter.add(this.up.multiply(farHeight * 0.5))
                .subtract(this.cameraRight.multiply(farWidth * 0.5)));
        ...
        nearTopRight = (nearCenter.add(this.up.multiply(nearHeight * 0.5))
                .add(this.cameraRight.multiply(nearWidth * 0.5)));
     ...
     */
    public Plane calcPlane(IVector3 pointOne, IVector3 pointTwo, IVector3 pointThree) {
        IVector3 spanU = pointTwo.subtract(pointOne);
        IVector3 spanV = pointThree.subtract(pointOne);
    ...
    private BoundingBox getOctreeNodeBox(OctreeNode<Integer> obj) {
        IVector3 ur = VectorMatrixFactory.newIVector3(
                ((OctreeNode) obj).getLowerLeft().get(0) + ((OctreeNode) obj).getLength(),
     ...
     */
    private IVector3[] calcCornerPointsOfBoundingBox(BoundingBox bBox) {
    ...
    /**
     * if visible scene nodes intersect an objectOctree, the objects visibility
     * is tested , builds a new triangle mesh for objects which are only
     * partially visible,
     * 
     * 
            ...
            if (boxesIntersect(sceneOctree.getBoundingBox(), objectOctree.getBoundingBox())) {
                ...
                if (toDraw.size() > 0) {
                    if (node.getContent().getClass() == TriangleMesh.class) {
                        ...
                        for (int i = 0; i < toDraw.size(); i++) {
                            for (int j = 0; j < toDraw.get(i).getNumberOfElements(); j++) {
                                ITriangle t = ((TriangleMesh) node.getContent())
                                        .getTriangle(toDraw.get(i).getElement(j));
                    ...
                    if (node.getContent().getClass() == PointCloud.class) {
                        ...
                        for (int i = 0; i < toDraw.size(); i++) {
                            for (int j = 0; j < toDraw.get(i).getNumberOfElements(); j++) {
                                ((PointCloud) contentToDraw).addPoint(
                                        ((PointCloud) node.getContent()).getPoint(toDraw.get(i).getElement(j)));
                            }
                        }
                    }
    ...
    public void computeVisibleScenePart(CgRootNode rootNode) {
        if (!initialised) {
            ...
            calcPlanesOfFrustum(Camera.getInstance().getNearClippingPlane(), Camera.getInstance().getFarClippingPlane(),
                    1.0); //
            rebuildOctree();
            visibleNodes.clear();
    ...
    public ArrayList<CgNode> traversalNode(CgNode node, ArrayList<CgNode> objects) {
        if (node.getNumChildren() > 0) {
            ...
            if (node.getContent() != null
                    && ((node.getContent().getClass() == TriangleMesh.class
                            || node.getContent().getClass() == PointCloud.class))
    ...
    /**
     * adds all (sceneOctree) nodes to a list which are visible or intersected
     * by the frustum
     * 
     * @param node
     ...
     * called when camera is moved, calculates new frustum and sets visibility
     * of objects
     */
    ...
    /**
     * builds the octree for the scene and generates octrees for every mesh of
     * the scene
     */
