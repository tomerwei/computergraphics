...
public class JoglFrame extends JFrame implements IRenderFrame<JoglRenderNode>,
		Observer {
	...
	public JoglFrame(CgApplication application) {
		...
		renderObjectManager = new JoglRenderObjectManager(
				application.getCgRootNode());
		...
		// Register controller
		Camera.getInstance().registerController(ControllerType.INSPECTION,
				new InspectionCameraController());
		Camera.getInstance().registerController(
				ControllerType.MOVEABLE_INSPECTION,
				new MoveableInspectionCameraController());
		Camera.getInstance().registerController(
				ControllerType.CAMERA_PATH,
				new CameraPathController(Camera.getInstance()
						.getCameraPathInterpolator()));
		Camera.getInstance().registerController(ControllerType.FIRST_PERSON,
	 ...
	 */
	public void registerRenderObjectsFactory(
			IRenderObjectsFactory<JoglRenderNode> factory) {
	...
	public void update(Observable o, Object arg) {
		...
		if (o instanceof AnimationTimer) {
			...
			if (MovieExport.getInstance().isRunning()) {
				takeScreenshot(MovieExport.getInstance()
