...
public class GPRenderer
{
	...
	public CgNode render3DMeshViewFromSolids()
	{
		...
		for (GPSolid s : solidList)
		{
			...
			rootNode.addChild(renderLinesOfSolid(s));
		}
	...
	private CgNode renderLinesOfSolid(GPSolid solid)
	{
		...
		mesh.getMaterial().setReflectionDiffuse(VectorFactory.createVector(Material.PALETTE1_COLOR3));
	...
	private void renderHorizontalSurfaces(ITriangleMesh mesh, GPLine[] basePair)
	{
		GPConfig gpConfig = this.data.getGPConfig();

		Vector floor_startA = basePair[0].getStart();
		Vector floor_endA = basePair[0].getEnd();
		Vector floor_startB = basePair[1].getStart();
		Vector floor_endB = basePair[1].getEnd();

		Vector normDirVecA = GPUtility.normalizeVector(GPUtility.substractOtherVector(floor_startA, floor_endA));
		Vector normDirVecB = GPUtility.normalizeVector(GPUtility.substractOtherVector(floor_startB, floor_endB));

		// Richtungen der Linien pruefen
		...
		double height)
	{
		...
		{
			mesh.addTriangle(index0, index3, index1);
			mesh.addTriangle(index1, index3, index2);
		}
	}
	...
	public CgNode render3DMeshViewFromGPDataType()
	{
		...
		for (Entry<String, List<GPLine>> e : layersMap.entrySet())
		{
			...
			System.out.println("Rendering " + e.getValue().size() + " elements of layer " + layerName);
	...
	private CgNode renderScaled3DMeshViewOfLayer(String layerName, List<GPLine> lineList)
	{
	...
	private CgNode render3DGridViewOfLayer(String layerName, List<GPLine> lineList)
	{
		...
		segment.getMaterial().setReflectionDiffuse(VectorFactory.createVector(Material.PALETTE1_COLOR0));
	...
	private LineSegments render3DGridWall(LineSegments segment, GPLine line)
	{
		...
		Vector upperEnd = new Vector(end.get(0), end.get(1), end.get(2) + gpConfig.getValue(GPConfig.WALL_TOP_HEIGHT));
	...
	private LineSegments render2DLineFromGPSurfaceList(List<GPLine> surfaceList)
	{
		...
		wallSegment.getMaterial().setReflectionDiffuse(VectorFactory.createVector(Material.PALETTE1_COLOR0));
	...
	public LineSegments renderWallFromDXFLineFormat(List<DXFLine> unitElements)
	{
		...
		unit.getMaterial().setReflectionDiffuse(VectorFactory.createVector(Material.PALETTE1_COLOR0));
		return unit;
	}
	...
	public LineSegments renderLineFromDXFLine(LineSegments segments, int index, DXFLine line)
	{
		...
		segments.addPoint(VectorFactory.createVector3(startPoint.getX(), startPoint.getY(), startPoint.getZ()));
		segments.addPoint(VectorFactory.createVector3(endPoint.getX(), endPoint.getY(), endPoint.getZ()));
