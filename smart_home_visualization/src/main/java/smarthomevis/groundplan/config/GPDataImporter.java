...
public class GPDataImporter {
	...
	public GPDataType importData(String dxfDocURI, GPConfig config) {
		this.resultData.setGPConfig(config);
	...
	private void extractWalls(DXFDocument dxf, GPConfig config) {
		...
		if (!layers.isEmpty()) {
			for (Entry<String, LineType> e : layers.entrySet()) {
				...
				System.out
						.println("Extracting " + lineType.toString().toLowerCase() + "s of layer '" + layerName + "'");
				...
				if (dxfLayer != null) {
					...
					if (entityList.size() > 0)
						...
						System.out.println("Could not Load DXFEntities: List is empty");
				} else
	...
	private List<DXFEntity> extractDXFLinesFromLayer(DXFLayer dxfLayer) {
		...
		List<DXFEntity> dxfEntities = dxfLayer.getDXFEntities(DXFConstants.ENTITY_TYPE_LINE);
	 ...
	 */
	private void handleLinesOfLayer(String layerName, List<DXFEntity> dxfLineList, LineType lineType, double scale) {
		...
		for (DXFEntity e : dxfLineList) {
			if (e instanceof DXFLine) {
				...
				GPLine surface = new GPLine(currentLineId,
						VectorFactory.createVector3(startPoint.getX() * scale, startPoint.getY() * scale,
								startPoint.getZ() * scale),
						VectorFactory.createVector3(endPoint.getX() * scale, endPoint.getY() * scale,
								endPoint.getZ() * scale));
